...
// This file is force-included only by the Communicator Compact target.
// Keep it valid for C translation units; the touch shim itself is C++ only.
#ifdef __cplusplus
...
// Standard (non-Plus) LilyGO T-Deck hardware constants.
...
// Wrap LilyGO's GT911 driver with two safeguards:
//  1. Re-establish the exact T-Deck peripheral power/I2C state before init.
//  2. If SensorLib rejects init even though the GT911 product-ID registers are
//     readable, fall back to the small register path needed for single-touch UI.
// This leaves the normal SensorLib path first and changes no radio/MeshCore code.
...
public:
  bool begin(TwoWire& wire,
    ...
    // LilyGO's own T-Deck example explicitly powers peripherals before Wire.
    pinMode(COMPACT_TDECK_POWERON, OUTPUT);
    digitalWrite(COMPACT_TDECK_POWERON, HIGH);
    pinMode(COMPACT_TDECK_TOUCH_INT, INPUT);

    // Re-create the bus deterministically in case another startup component
    // touched Wire after TDeckBoard::begin(). 100 kHz matches Arduino Wire's
    // default used by LilyGO's example.
    wire.end();
    delay(10);
    ...
    // First use LilyGO's full driver. The Compact target pins SensorLib to the
    // exact pre-config rewrite revision whose GT911 header matches T-Deck.
    ...
    // If the controller is plainly present, do not throw away working touch
    // merely because the higher-level driver rejected its configuration.
    if (id5dok || id14ok) {
      _compact_addr = id5dok ? GT911_SLAVE_ADDRESS_L : GT911_SLAVE_ADDRESS_H;
      _compact_fw = id5dok ? fw5d : fw14;
      _compact_raw_fallback = true;
      Serial.printf("[compact-touch] SensorLib init failed; raw GT911 fallback active at 0x%02X\n", _compact_addr);
      return true;
    }
  ...
  uint32_t getChipID() {
    return _compact_raw_fallback ? 911 : TouchDrvGT911::getChipID();
  }

  uint16_t getFwVersion() {
    return _compact_raw_fallback ? _compact_fw : TouchDrvGT911::getFwVersion();
  }
...
// UITask.cpp already declares `TouchDrvGT911 tdeck_touch;`. Replace only that
// type in the Compact build after the real SensorLib class above is defined.
