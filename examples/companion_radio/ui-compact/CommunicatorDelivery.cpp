...
void CommunicatorAppScreen::reconcileDirectSendState() {
  ...
  for (int i = 0; i < MESSAGE_CACHE; ++i) {
    ...
    if (!the_mesh.isCompactAckPending(m.delivery_ack)) {
      m.send_state = SEND_SENT;
      m.delivery_ack = 0;
      m.delivery_deadline_ms = 0;
      changed = true;
      if (_route == ROUTE_CHAT && strcmp(m.origin, _active_name) == 0) _task->showAlert("Sent", 650);
      continue;
    }
...
void CommunicatorAppScreen::drawDirectSendOverlay(DisplayDriver& d) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (m.outgoing && m.send_state == SEND_SENDING) {
      ...
      // line while an ACK is outstanding; the normal Sent/age line reappears
      // automatically once reconciliation marks the message acknowledged.
      d.setColor(bubble);
